  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "drivers", uid), (snap) => {
      if (snap.exists()) {
        ...
        setVille((prev) => prev || data.city || "");
        setSnapchat((prev) => prev || data.snapchat || "");
        ...
        <div className="max-w-md mx-auto space-y-4">
          ...
          <form onSubmit={handlePublierAnnonce} className="bg-white rounded-2xl border border-[#EEE8DD] p-5 space-y-4">
            <label className="w-full h-44 border border-dashed border-[#CBCBCB] rounded-xl flex items-center justify-center cursor-pointer overflow-hidden text-gray-400">
              ...
              <input type="file" accept="image/*" hidden onChange={(e) => e.target.files?.[0] && setImage(e.target.files[0])} />
            </label>
            <p className="text-xs text-gray-400 text-center -mt-2">
              Touchez pour changer la photo (une seule photo publiée)
            </p>
            <textarea placeholder="Description (véhicule, disponibilités...)" value={descriptionActuelle}
              onChange={(e) => setDescriptionActuelle(e.target.value)} rows={4}
    ...
    <div className="min-h-screen bg-[#F8F6F2] pb-40 px-4 pt-6">
      ...
      <form onSubmit={handleConfirmer} className="max-w-md mx-auto bg-white rounded-2xl border border-[#EEE8DD] p-5 space-y-4">
        ...
        <input type="tel" placeholder="Numéro de téléphone" value={phone} onChange={(e) => setPhone(e.target.value)}
          className="w-full border border-[#EEE8DD] rounded-xl px-4 py-3 outline-none focus:border-[#FF6E14]" />
        <input type="text" placeholder="Snapchat (optionnel)" value={snapchat} onChange={(e) => setSnapchat(e.target.value)}
          className="w-full border border-[#EEE8DD] rounded-xl px-4 py-3 outline-none focus:border-[#FF6E14]" />
        <input type="text" placeholder="Ville / zone de circulation" value={ville} onChange={(e) => setVille(e.target.value)}
          className="w-full border border-[#EEE8DD] rounded-xl px-4 py-3 outline-none focus:border-[#FF6E14]" />
        <textarea placeholder="Description (véhicule, disponibilités...)" value={description}
